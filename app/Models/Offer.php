...
class Offer extends Model
{
    ...
    /*
     * Legacy: reserved bleibt nur als technische Alt-Konstante,
     * wird aber nicht mehr aktiv im System ausgewählt.
     */
    ...
    public const EXPIRING_RESERVATION_STATUSES = [
        self::STATUS_OFFER,
    ];
    ...
    public const STATUS_LABELS = [
        self::STATUS_OFFER => 'Angebot',
        self::STATUS_IN_PROGRESS => 'In Bearbeitung',
        self::STATUS_READY => 'Abholbereit',
    ...
    protected static function booted(): void
    {
        static::creating(function (Offer $offer): void {
            if (! $offer->offer_number) {
                ...
                $lastNumber = Offer::query()
                    ->where('offer_number', 'like', 'ANG-' . $year . '-%')
                    ->orderByDesc('id')
                    ->value('offer_number');

                $lastSequence = $lastNumber
                    ? (int) substr($lastNumber, -3)
                    : 0;
