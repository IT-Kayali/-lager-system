    ...
    <div class="offer-preview-layout">
        <section class="premium-card offer-preview-main">
            <div class="premium-toolbar">
                <div><h2 data-csp-style="s-90f6599a">{{ $offer->offer_number }}</h2><div class="premium-muted" data-csp-style="s-c38d12ec"><span>{{ $offer->customer?->company_name }}</span><span>·</span><x-customer-group-badge :group="$offer->customer?->group" /><span>·</span><span>{{ $offer->templateLabel() }}</span></div></div>
                <div data-csp-style="s-09adedd2">
                    ...
                    @if (auth()->user()?->isManager()) <form method="POST" action="{{ route('offers.destroy', $offer) }}" data-confirm="Angebot wirklich löschen?">@csrf @method('DELETE')<button class="premium-btn" type="submit" data-csp-style="s-3ef31e4d"><i class="bi bi-trash"></i> Löschen</button></form> @endif
                </div>
            </div>

            @if (auth()->user()?->isManager() && ! $offer->isFinal())
                <form method="POST" action="{{ route('offers.status', $offer) }}" data-csp-style="s-d9fcbe51">@csrf @method('PUT')<div class="premium-form-grid"><div class="premium-form-field"><label>Status ändern</label><select name="status" class="premium-select">@foreach (['offer'=>'Angebot','in_progress'=>'In Bearbeitung','ready'=>'Abholbereit','completed'=>'Erledigt','cancelled'=>'Storniert','reservation_expired'=>'Reservierung abgelaufen'] as $value=>$label)<option value="{{ $value }}" @selected($offer->status === $value)>{{ $label }}</option>@endforeach</select></div><div class="premium-form-field" data-csp-style="s-1e496597"><button class="premium-btn gold" type="submit"><i class="bi bi-arrow-repeat"></i> Status speichern</button></div></div></form>
            @elseif (auth()->user()?->isSales() && $offer->status === \App\Models\Offer::STATUS_OFFER)
                <form method="POST" action="{{ route('offers.status', $offer) }}" data-csp-style="s-d9fcbe51">@csrf @method('PUT')
            ...
            @else
                <div data-csp-style="s-d9fcbe51"><span class="premium-badge ok">Status: {{ $offer->statusLabel() }}</span>@if(auth()->user()?->isSales() && $offer->status !== \App\Models\Offer::STATUS_OFFER)<span class="premium-muted" data-csp-style="s-69c60917">An Lager übergeben · nicht mehr bearbeitbar</span>@endif</div>
            @endif
            ...
            <div data-csp-style="s-761d63a5">Gesamt: {{ \App\Support\GermanNumber::format($offer->total) }} €</div>
        </section>
