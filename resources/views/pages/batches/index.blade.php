<x-layouts.premium title="Chargen & FIFO" subtitle="Wareneingänge, Batchnummern und FIFO-Lagerlogik.">
    ...
    <section class="batches-page-actions">
            ...
            <form method="GET" action="{{ route('batches.index') }}" class="batches-search-modern">
                <div class="batches-search-field">
                    ...
                    <input
                        ...
                        value="{{ $search ?? '' }}"
                        class="premium-input"
                        placeholder="Charge, Produkt oder Hersteller suchen..."
                    >
                </div>
        ...
        <a href="{{ route('batches.create') }}" class="premium-btn gold batches-create-btn">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="batch-product-cell">
                                    ...
                                    <span>{{ $batch->product?->product_code ?: 'Kein Produktcode' }}</span>
                                </div>
                            </td>

                            <td>
                                <span class="batch-quantity {{ $quantity <= 0 ? 'empty' : '' }}">
                                    {{ number_format($quantity, 2, ',', '.') }}
                                </span>
                                <span class="batch-unit">{{ $batch->product?->unit ?? '' }}</span>
                            </td>

                            <td>
                                @if ($expiresAt)
                                    <span class="batch-expiry {{ $isExpired ? 'expired' : ($isExpiringSoon ? 'soon' : 'regular') }}">
                                        <i class="bi {{ $isExpired ? 'bi-x-circle' : ($isExpiringSoon ? 'bi-exclamation-circle' : 'bi-calendar-check') }}"></i>
                            ...
                            <td>
                                <div class="premium-actions batches-actions">
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="batches-empty-state">
                                    ...
                                    <span>Lege eine neue Charge an, um FIFO-Bestand aufzubauen.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <style>
        ...
        .batches-empty-state i {
            font-size: 34px;
            color: #8a6a00;
        }

        .batches-empty-state strong {
            color: #111111;
            font-size: 17px;
        }
        ...
        .batches-table th:nth-child(6),
        .batches-table td:nth-child(6) {
            text-align: center;
        }
